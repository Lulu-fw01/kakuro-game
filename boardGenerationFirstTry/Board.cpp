...
Board::Board(int height, int width) {
    m_height = height;
    m_width = width;
    m_cells = std::vector<std::vector<EmptyCell *>>(height, std::vector<EmptyCell *>(width, new EmptyCell));
}
...
bool Board::isValid(int row, int column) const {
    if (row == m_cells.size() - 2 || column == m_cells[0].size() - 2) {
        return false;
    }
...
void Board::setCell(int row, int column, EmptyCell *cell) {
